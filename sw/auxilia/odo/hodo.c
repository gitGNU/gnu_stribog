...
static void moram_iube(void){TCNT0=0;TIFR|=(1<<TOV0);}
static unsigned char mora(void)
...
static inline unsigned char ch_redde(void)
{unsigned char c=capta[cauda++];if(cauda>=sizeof(capta))cauda=0;return c;}
static unsigned char mittenda_redde(void)
{if(i>=sizeof(missa))pone_mittenda();return missa[i++];}
static void cape(void){if(!(UCSRA&(1<<RXC)))return;ch_adde(UDR);}
static void mitte(void)
{static unsigned char v,p=3;char _=caput!=cauda;if(!(UCSRA&(1<<UDRE)))return;
 if(_){if(++v==p)v=0;}else{v=0;if(mora())return;if(i==sizeof(missa))moram_iube();}
...
Copyright (C) 2007 D.Ineiev <ineiev@users.sourceforge.net>, super V 93*/
