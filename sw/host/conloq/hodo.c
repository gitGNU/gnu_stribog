...
{static double t_prev=-1,dt0,t_mess=-1;static long ts0=-1;
 static int dt0v,filter_count;int r=0;
 if(i<0){if(!(c&(1<<6)))i=0;t_mess=-1;return 0;}if(t_mess<0)t_mess=t;
 if(!silent)output_next=!0;s[i++]=c&0x3F;
 if(i>sizeof(s)){error("hodo message too long t=%f\n",t);i=0;t_mess=-1;return!0;}
 if(!(c&(1<<6)))
 {if(i>5)
  {unsigned _,d;long ts;long l=0;unsigned char sum;i-=6;
   for(_=sum=0;_<6;_++)sum^=s[i+_];
   if(sum){error("hodo checksums don't match (diff=%2.2X) t=%f\n",sum,t);return!0;}
   ...
   {unsigned long long t_full=ts+(((unsigned long long)t_loops)<<16);
    ...
    {if(dt0v)
     {double dt=t_mess-t_full/freq0-dt0;
      ...
      }if(output_next)printf("ododt: %.8f %llu %.8f %.8f\n",t,t_full,dt,dt0);
     }else 
     {if(ts0>=0)
      {dt0v=!0;dt0=t_mess-t_full/freq0;
       if(output_next)printf("ododt0: %.8f %llu %.8f\n",t,t_full,dt0);
      }
     }
    }
    if(output_next)
    {double t_corrected=t_full/freq0+dt0;
     printf("odo: %.8f %llu %llu %li %li\n",t_corrected,t_full,x,ts,ts0);
    }output_next=0;
   }_0=_;ts0=ts;t_prev=t_mess;
  }else 
  {error("hodo message too short t=%.8f (%i bytes: %2.2X %2.2X %2.2X %2.2X %2.2X)\n",
