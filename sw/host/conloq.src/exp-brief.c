/*decoding stribog messages with interactive output control
...
#include"get_u.h"
static const double nominal_ticks_freq=14745600.;
static double freq=14745600.+8550;static FILE*gps_extracted;
static unsigned leaps;
static int period=1,verbous=0;
static unsigned long long time_stamp;
static int
flush_on_need(void)
{if(get_verbosity()<normal_verbosity&&
  get_interaction_mode()==interactive_mode)
 {fflush(stdout);return!0;}
 return 0;
}
void
adjust_frequency(double dF)
{freq=nominal_ticks_freq+dF;
 if(get_verbosity()>=pretty_verbose)
  printf("frequency is adjusted by %.3f Hz\n",dF);
}
static double
...
init_exp(int p)
{init_gps();period=p;verbous=get_verbosity();
 if(get_interaction_mode()!=interactive_mode)
  gps_extracted=/*FIXME customize this*/fopen("gps.log","wb");
 if(verbous>=pretty_verbose)
  printf("ADC messages decimation number is %i\n",period);
 return 0;
}
void
close_exp(void)
{close_gps();if(gps_extracted)fclose(gps_extracted);}
...
{unsigned _2048,t[3];
 ...
 if(get_verbosity()>=relatively_mute)printf("temp:");
 if(flush_on_need())return;
 printf(" %.8f %u %u %u %u\n",mcu_stamp(),
  t[0],t[1],t[2],_2048);
}static double latest_pps=-1;
double 
correct_utc_second(double utc,double t_mcu)
{static double dutc=-3.*24*3600;int i,j;double dt;
 if(latest_pps<0||get_interaction_mode()==interactive_mode)
  return t_mcu;
...
{static unsigned t0,t0f,loops,v,vf;static double df;
 ...
 if(get_interaction_mode()==interactive_mode)
 {if(dt)d/=dt;t0=t;v=!0;
  if(!stribog_message_turned_on(pps_message))return 0;
  if(get_verbosity()>=relatively_mute)printf("pps:");
  if(flush_on_need())return 0;
  printf("  %.8f %.0f %g\n",mcu_time(tmcu),
...
{static int i;int a[3],b[3],w[3],T,j,turned_on;unsigned long t,x;
 ...
 if(period<0)return 0;/*FIXME impossible condition*/
...
{unsigned long n=0,k,sh=0;static unsigned long n0;int r=0;
 if(get_verbosity()<pretty_verbose)return 0;
 printf("gps_cont: %2.2X %2.2X %2.2X %2.2X %2.2X\n",s[7],s[8],s[9],s[10],s[11]);
 s+=12;k=*--s;
 if(verbous>1)printf("gps_message_length: %.8f %lu\n",mcu_stamp(),k);
...
{unsigned long tmax=get_u(s),adc_missed=get_u(s+=4),accel_errors=get_u(s+=4),
 ...
 if(get_verbosity()>=relatively_mute&&turned_on)printf("stat:");
 if(flush_on_need())return 0;
...
{unsigned long crc,cr;crc=form_crc(s,(size>>2)-1);cr=get_u(s+size-4);
 ...
 {case 8:exp_pps(s);break;case 12:exp_temp(s);break;
  case 16:
  {int r=check_gps_overflows(s);
   r|=exp_gps(mcu_stamp(),s,gps_extracted);return r;
  }
